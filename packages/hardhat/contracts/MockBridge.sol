...
contract MockBridge is IBridge {
  event TokenTransferred(address indexed token, uint64 indexed chainId, address indexed target, uint256 amount);
  ...
  event CrossChainCalled(uint64 indexed chainId, address indexed target, bytes data);
  ...
  function transferToken(IERC20Metadata token, uint64 chainId, address target, uint256 amount) external override {
  ...
  function callCrossChain(uint64 chainId, address target, bytes calldata data) external override {
