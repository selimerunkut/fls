...
describe("PythPriceOracle", function () {
  ...
  before(async () => {
    ...
    pythPriceOracle = (await PythPriceOracleFactory.deploy(owner.address, pyth.target)) as PythPriceOracle;
  ...
  describe("Get Price", function () {
    beforeEach(async () => {
      ...
      const updateData = [
        await pyth.createPriceFeedUpdateData(
          ETH_PRICE_FEED_ID,
          TOKEN_PRICE,
          10 * 100000,
          -5,
      ...
      await pythPriceOracle.updatePrice(updateData, token1.address, usdToken.address, {
